...
async function findGoogleStudentCode(spreadsheetId, email) {
  ...
  for (var i = 0; i < sheets.length; i++) {
    ...
    if (!title) continue;
    var range = quoteGoogleSheetName(title) + '!A2:D500';
...
async function signInGoogleStudent() {
  ...
  try {
    ...
    var match = await findGoogleStudentCode(spreadsheet.id, email);
    if (!match) throw new Error('No student code was found for ' + email + '. Please use your normal code or ask your teacher.');
